...
const doubledNumber = doubleUsingCallback(numbers, (s) => s * 2);
...
const manipulate = manipulateString(`Hello Gunjan`); // function iss string ko upper case m convert krra h
const logString = manipulate();
...
const books = [
  ...
  const sortedTitles = books.map(({ title }) => title).sort();
...
fetch('https://jsonplaceholder.typicode.com/posts/123456789')
  ...
  .then(data => {
    console.log(data);
    document.getElementById('result').innerHTML = `Data retrieved successfully: ${JSON.stringify(data)}`;
  })
  .catch(error => {
    console.error('Error fetching data:', error);
